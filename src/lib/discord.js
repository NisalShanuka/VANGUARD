...
export async function sendDiscordDM({ userId, embed }) {
    ...
    try {
        ...
        if (!dmData.id) return { ok: false, reason: 'failed_to_open_dm', data: dmData };

        // Send Message
        const msgRes = await fetch(`${DISCORD_API_BASE}/channels/${dmData.id}/messages`, {
            method: 'POST',
            headers: {
                Authorization: `Bot ${botToken}`,
                'Content-Type': 'application/json'
        ...
        return { ok: false, reason: 'failed_to_send_message' };
    } catch (e) {
